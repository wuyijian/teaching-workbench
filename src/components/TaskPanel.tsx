...
Structure (严格遵守):
一、【课堂反馈】（多个学生也放在一起）
二、课前检测： 简单客观表达课前检测内容以及结果 （如无可不写）
三、核心内容： 简述本节课讲了什么（如：文言文虚词、名著阅读、考纲作文）。
四、课堂表现： 客观描述孩子在课上的参与度、思维活跃度，分学生写
五、课后检测： 简单客观表达课前检测内容以及结果 （如无可不写）
六、课后叮嘱： 针对性指出孩子的问题以及要注意或者完成的事项，尽量客观 （如无可不写）

Tone & Principles:
客观真实： 不夸大，不敷写，避免空洞的"表现很好"。
分段呈现： 使用自然段落和中文序号（一、二、三），每段之间空一行，方便家长手机阅读。
纯文字输出： 严禁使用任何 Markdown 语法，包括 **粗体**、__下划线__、# 标题、- 列表、> 引用、\`代码\` 等符号，只输出纯文字。
...
一、【试卷分析】（多个学生也放在一起）
二、总体情况：简述得分与整体表现
三、错题类型分析：归纳主要错误类型（如概念混淆、计算失误、审题不仔细等）
四、知识点薄弱项：指出需要重点复习的知识点或题型
五、改进建议：给出具体可行的复习方向与练习建议

Tone & Principles:
客观精准：仅基于提供的批改结果，不编造数据。
分段呈现：使用自然段落和中文序号（一、二、三），每段之间空一行，方便查阅。
纯文字输出：严禁使用任何 Markdown 语法，包括 **粗体**、__下划线__、# 标题、- 列表、> 引用、\`代码\` 等符号，只输出纯文字。
